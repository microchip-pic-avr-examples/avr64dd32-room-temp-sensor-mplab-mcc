...
bool _writeReadMLX90632(MLX90632_Register reg, uint8_t* memory, uint8_t size)
{
    uint8_t i2cBuffer[2];
    ...
    //Queue I2C
    bool success = TWI0_WriteRead(MLX90632_I2C_ADDR_BASE, &i2cBuffer[0], 2, &memory[0], size);
...
bool _MLX90632_loadConstantsFromDevice(void)
{
    ...
    //Queue I2C Operation
    bool success = _writeReadMLX90632(EE_P_R_LOW, &dataPool[0], 16);
    ...
    success = _writeReadMLX90632(EE_EA_LOW, &dataPool[0], 24);
    ...
    //Skip to Ha   
    success = _writeReadMLX90632(EE_HA_LOW, &dataPool[0], 4);
...
{  
    ...
    //Get ID
    bool success = _writeReadMLX90632(MLX90632_ID0, &readBuffer[0], 6);
...
bool MLX90632_getRegister(MLX90632_Register reg, uint16_t* result)
{
    ...
    bool success = _writeReadMLX90632(reg, &rValues[0], 2);
    
    if (!success)
        return false;
        
    //Create 16-bit result
    *result = CREATE_16BIT(rValues[0], rValues[1]);
...
bool MLX90632_getResults(void)
{
    ...
    bool success = _writeReadMLX90632(MLX90632_REG_STATUS, &statusBytes[0], 2);
    ...
    success = _writeReadMLX90632(MLX90632_RAM4_START, &dataBuffer[0], 12);
...
{   
    uint8_t wData[4];
    wData[0] = (reg & 0xFF00) >> 8;
    wData[1] = (reg & 0xFF);
    wData[2] = (data & 0xFF00) >> 8;
    wData[3] = (data & 0xFF);
    
    //Queue I2C
    bool success = TWI0_Write(MLX90632_I2C_ADDR_BASE, &wData[0], 4);
